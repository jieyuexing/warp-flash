...
fn first_zero_state_matches_welcome_design_copy() {
    App::test((), |mut app| async move {
        ...
        for expected in [
            warp_i18n::localize_ref("Welcome to Warp"),
            warp_i18n::localize_ref("What’s different about Warp"),
        ...
        assert!(!rendered.contains(warp_i18n::localize_ref("What's new")));
...
fn custom_endpoint_section_is_hidden_until_an_endpoint_is_configured() {
    App::test((), |mut app| async move {
        ...
        app.read(|ctx| {
            ...
            .join("\n");
            assert!(!rendered.contains(warp_i18n::localize_ref("Custom endpoints")));
        });
