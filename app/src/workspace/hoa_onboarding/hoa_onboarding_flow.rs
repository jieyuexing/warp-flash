    ...
    fn render_inbox_callout(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let formatted = FormattedText::new([FormattedTextLine::Line(vec![
            FormattedTextFragment::plain_text(
                warp_i18n::localize_ui("Warp pipes through notifications from any CLI coding agent into a unified notification center that works across all coding agents and harnesses. ").into_owned(),
            ),
            learn_more_fragment,
        ])]);
